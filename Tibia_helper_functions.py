...
def add_plane(name):
    bpy.ops.mesh.primitive_plane_add(size=120, enter_editmode=False)
...
def coll_list():
    my_list = ["Tibia_Landmarks", "Tibia_Axes", "Tibia_Planes", "Tibia_Projections", "Tibia_Measurements"]
    return my_list
...
def create_ankle_point():
    try:
        delete_obj(bpy.data.objects["Ankle Center"])
    except:
        pass

    copy_object(bpy.data.objects["Point1 (Ankle Center)"], "P1 for ankle point")
    copy_object(bpy.data.objects["Point2 (Ankle Center)"], "P2 for ankle point")
    make_axis([bpy.data.objects["P1 for ankle point"], bpy.data.objects["P2 for ankle point"]], "AxisForAnatomicalPoint")
    origin_to_geometry(bpy.data.objects["AxisForAnatomicalPoint"])
    cursor_to_obj(bpy.data.objects["AxisForAnatomicalPoint"])
    add_single_vert("Ankle Center")
    delete_obj(bpy.data.objects["AxisForAnatomicalPoint"])
